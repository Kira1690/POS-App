...
export const AuthProvider: React.FC<AuthProviderProps> = ({ 
  ...
  // Role-based utilities (use server role directly — 8-role RBAC system)
  const hasRole = useCallback((role: UserRole): boolean => {
    return (state.user?.role as unknown as UserRole) === role;
  }, [state.user?.role]);

  const hasAnyRole = useCallback((roles: UserRole[]): boolean => {
    return roles.includes(state.user?.role as unknown as UserRole);
  }, [state.user?.role]);
  ...
  const canAccessResource = useCallback((requiredRoles: UserRole[]): boolean => {
    ...
    return requiredRoles.includes(state.user?.role as unknown as UserRole);
  }, [state.isAuthenticated, state.user, state.user?.role]);
